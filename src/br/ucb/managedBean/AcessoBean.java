...
public class AcessoBean implements Serializable {
	...
	public String login() {
		try {
			GenericWorker<Acesso, String> regHBR2 = new GenericWorker<Acesso, String>(Acesso.class);
			acesso = (Acesso) regHBR2.consulta(this.email);
			regHBR2.finalize();
			pessoa = acesso.getPessoa();
			if (email.equals(acesso.getEmail()) && senha.equals(acesso.getSenha())) {
				this.mensagemUsuario = "";
				return "sucesso";
			} else {
				this.mensagemUsuario = "Dados inválidos.";
				FacesContext context = FacesContext.getCurrentInstance();
				context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERRO!", "Dados inválidos."));
				return "insucesso";
			}
		} catch (Exception e) {
